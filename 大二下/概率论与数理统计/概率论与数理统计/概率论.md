...
| 事件相等       | $A=B$, $A \subset B$ and $A\supset B$ |
| 事件的并（和） | $A\cup B$                          |
| 事件的交（积） | $A\cap B=AB$                       |
| 事件的差       | $A-B$                              |  |
...
$$
f_n(A)=\frac{n_A}{n}=\frac{\text{事件A发生的频数}}{\text{试验总次数}}
$$
...
2. 规范性：$f_n(\Omega)=1$;
3. 可加性：若$A_1,A_2,\cdots,A_m$为m个两两不相容的事件，则  $f_n(A_1\bigcup A_2\bigcup\cdots\bigcup A_m)=f_n(A_1)+f_n(A_2)+\cdots +f_n(A_m)$
...
$$
P(A)=\sum_{j=1}{k}{P(e_{i_j})}=\frac{k}{n}=\frac{\text{A包含的基本事件数}}{\Omega\text{中基本事件总数}}
$$
...
**定义**：所有可能取值为有限个或可列无限个的随机变量称为**离散型随机变量**.若X为离散型随机变量，不妨设其取值为$x_1,x_2,\cdots,x_n,\cdots,$令$P\{\omega|X(\omega)=x_k\}\overset{\text{简记}}{=}P\{X=x_k\}\overset{\Delta}{=}p_k,k=1,2,\cdots,$称$p_k=P\{X=x_k\},k=1,2,\cdots$为离散型r.v.X的**分布律**或**分布列**或**概率分布**
...
$$
P\{a<X<b\}=\int_a^bf(x)dx=F(b)-F(a)\text{;两边取等或不取都一样}
$$
$$
f(x)=\frac{F(x)}{dx}\text{;不需要考虑间断点的导数}
$$
...
| 6   | $P\{X=a\}=0\Leftrightarrow$不可能事件   | $P\{X=a\}=0\nLeftrightarrow$不可能事件 |
