...
**雅各布家族** *雅各布·伯努利(Jacob Bernoulli,1654-1705)*给出了gutushuguangwenti的详尽解法并证明了大数定律的一个定理，1713年（时已逝）出版《猜度术》。侄子尼古拉·伯努利（Nicolaus Bernoulli,1687-1759)提出了著名的圣彼得堡问题（1738）
...
1. 由部分试验结果组成的集合称为**随机事件**，简称为**事件**。常用大写英文字母A、B、C等表示
     随机事件就是样本空间的子集。$ 事件A发生\Longleftrightarrow \omega \in A$。
2. 几个特殊事件：
...
1. 事件的包含$A\subset B  \ or\  B\supset A$
2. 事件相等$A=B \ \ A \subset B \ A\supset B$
3. 事件的并（和）$A\cup B$
4. 事件的交（积）$A\cap B=AB$
5. 事件的差($A-B$)
6. 互不相容事件$A\cap B=\emptyset$
7. 对立事件$\bar{A}$
8. 完备事件组[^完备事件组]
...
$$
![示例图片](https://note.youdao.com/favicon.ico)以上内容在有道云笔记上写成
...
若事件A包含k个基本事件，即$A={e_{1_1,}e_{i_2},\cdots,e_{i_k}}$,于是[^141]
$$
P(A)=\sum_{j=1}{k}{P(e_{i_j})}=\frac{k}{n}=\frac{A包含的基本事件数}{\Omega中基本事件总数}
$$
[^141]: 古典概型中**事件概率的计算公式**也称为**概率的古典定义**。古典概率模型(classical probability model),简称古典概型，又称等可能概率模型、
...
$$
P(B_i|A)=\frac{P(B_i)P(A|B_i)}{\sum\limits_{j=1}^{n}P(B_j)P(A|B_j)},i=1,2,\dots,n(2)
$$
上式为贝叶斯公式,由全概率公式可知分母为P(A)，由条件概率公式可知分子为$P(AB_i)$

### 2-3 独立性

定义1：当P(B)>0时，若事件B的发生对事件A发生的概率没有影响，即`P(A|B)=P(A)`，则称**事件A对事件B独立**.

定义2：当P(B)>0时，若P(A|B)=P(A)，则称A与B**相互独立**，简称**A、B独立**<br>当P(A)>0时，若P(B|A)=P(B)，则称A与B**相互独立**，简称**A、B独立**
...
**定义**：所有可能取值为有限个或可列无限个的随机变量称为**离散型随机变量**.<br>若X为离散型随机变量，不妨设其取值为$x_1,x_2,\cdots,x_n,\cdots,$令$P\{\omega|X(\omega)=x_k\}\overset{简记}{=}P\{X=x_k\}\overset{\Delta}{=}p_k,k=1,2,\cdots,$称$p_k=P\{X=x_k\},k=1,2,\cdots$为离散型r.v.X的**分布律**或**分布列**或**概率分布**
...
吧贝努利试验重复独立地进行n次，所得的试验称为**n重贝努利试验**。即n重贝努利试验由n次基本试验构成，具有以下特点：
...
- **二项分布**（Binomial Distribution)<br>$X \sim B(n,p)$——$P\{X=k\}=C_n^kp^k(1-p)^{n-k},k=0,1,2,\cdots,n$[^321]<br>表示n次实验中**第k次**发生的概率
- **几何分布**（Geometric Distribution）<br>$X\sim G(n,p)$——$P\{X=k\}=(1-p)^{k-1}p,k=1,2,\cdots,0<p<1$<br>在贝努利试验中，事件A在每次试验中发生的概率均为p（0<p<1)，以X表示A**首次**发生时**需要的试验次数**，那么X的分布律为
- **泊松分布**（Poisson Distribution)[^322]<br>$X\sim P(\lambda)$——$P\{X=k\}=\frac{\lambda^k}{k!}e^{-\lambda},k=0,1,2,\dots$<br>若r.v.X的所有取值为一切非负整数，且其分布律为上式，其中$\lambda>0$为常数，泊松分布在排队论和随机系统中非常有用
...
[^321]:二项分布的最可能发生次数$m=\begin{cases} (n+1)p和(n+1)p-1 & (n+1)p整数\\ [(n+1)p] 取整& (n+1)p不是整数\end{cases}$

[^322]:泊松分布的最可能发生次数$m=\begin{cases} \lambda和(\lambda-1) & \lambda整数\\ [\lambda] 取整&\lambda不是整数\end{cases}$
...
2. 非负规范性：对任意实数x，有$0\leq F(x\leq 1;$ $ F(-\infty)=\lim\limits_{x\to-\infty }F(x)=0,F(+\infty)=\lim\limits_{x\to+\infty}F(x)=1$
...
1. 如果 $X \sim N\left(\mu, \sigma^{2}\right)$ 且a与b为实数，那么 $a X+b \sim N\left(a \mu+b,(a \sigma)^{2}\right)$
2. 如果 $X \sim N\left(\mu_{X}, \sigma_{X}^{2}\right)$ 与 $Y \sim N\left(\mu_{Y}, \sigma_{Y}^{2}\right)$ 是统计独立的正态随机变量，那么：
...
3. 如果 $X \sim N\left(0, \sigma_{X}^{2}\right)$ 和 $Y \sim N\left(0, \sigma_{Y}^{2}\right)$ 是独立常态随机变量，那
...
计算标准正态分布时，可以使用查表，利用计算机等方法来实现Excel函数为`NORMDIST`

 [正态分布excel表示例](概率论与数理统计.assets\正态分布.xlsx)
...
**定义**：设X为随机变量，其分布函数定义为：$F(x)=P\{X\leq x\},-\infty<x<+\infty$

$X\leftrightarrow Q$,$x\leftrightarrow Q_0$$F(Q_0)=P\{Q\leq Q_0\},Q_0\in \mathbb{R}$

$R\rightarrow R^2$,$Q\leftrightarrow (X,Y)$,$Q_0\leftrightarrow (x,y)$:
$$
...
<p hidden >
特别，当$g(x)$为严格单调增可导时，比如$g(x)$为严格单调可导时，且其反函数为$h(y)$，于是$h'(y)\geq 0$.当X取遍所有可能取值时$Y=g(X)$的取值范围为$[\alpha,\beta]$.于是：
...
1. **串联**系统的寿命Z为Z=max{X,Y}
2. **并联**系统的寿命Z为Z=min{X,Y}
...
4. $E(X+Y)=E(X)+E(Y)$;<br>$E(c_1X_1+\cdots+c_nX_n)=c_1E(X_1)+\cdots+c_nE(X_n)$数学期望是线性算子
5. 若X与Y独立，则$E(XY)=E(X)E(Y)$.<br>若$X_1,X_2,\dots,X_n$相互独立，则$E(X_1X_2\cdots X_n)=E(X_1)E(X_2)\cdots E(X_n)$
...
- 相关系数并不是刻画了一般的函数关系，而只是刻画了“线性”关联的程度，当且仅当X与Y有严格的线性关系时，才有$|\rho_{XY}|$达到最大值1.即使X与Y有某种确定的函数关系时，也可能会有Cov(X,Y)=0
...
**特别**$Cov(X,Y)=E\{[X-E(X)][Y-E(Y)]\}$为1+1阶混合中心矩
...
[^依概率收敛]:设 $Y_{1}, Y_{2}, \cdots, Y_{n}$ 是一个随数,若对于任意正数 $\varepsilon$有 $\lim_{n \rightarrow \infty} P\left\{\left|Y_{n}-a\right|<\varepsilon\right\}=1$则称序列 $Y_{1}, Y_{2}, \cdots, Y_{n}$依概率收?于 a,记为$Y_{n} \stackrel{P}{\longrightarrow} a$
...
$$
定理二**表明**:无论各个随机变量 $X_{1}, X_{2}, \cdots, X_{n}, \cdots$ 服从什么分布,只要满足定理的条件, 那么它们的和 $\sum_{k=1}^{n} X_{k}$当 $n$ 很大时,近似地服从正态分布.
...
$$
定理三**表明**:正态分布是二项分布的极限分布, 当n充分大时, 可以利用该定理来计算二项分布的概率.
